...
{
	/// <summary>
	/// Выполнение запроса <see cref="CreateMedicationScheduleCommand"/>
	/// </summary>
	internal class CreateMedicationScheduleHandler : LoggerWrapper, IRequestHandler<CreateMedicationScheduleCommand, int>
	{
			...
			using (_ = base.BeginLoggingScope())
			{
				...
				if (list.Any(x => x.PersonMedicamentId == model.PersonMedicamentId &&
					(model.DayTimeTypeId.HasValue ? x.DayTimeTypeId == model.DayTimeTypeId :
						x.Time == model.Time)))
				{
					throw new BadRequestException($"Уже имеется похожее расписание с " +
						$"{nameof(DomainExt.MedicationSchedule.PersonMedicamentId)} = {model.PersonMedicamentId} и " +
						(model.DayTimeTypeId.HasValue ? $"{nameof(DomainExt.MedicationSchedule.DayTimeTypeId)} = {model.DayTimeTypeId}" :
							$"{nameof(DomainExt.MedicationSchedule.Time)} = {model.Time}"));
				}
